...
const Service = () => {
    ...
    const handleSubmit = (event) => {
        ...
        const newUser = { full_name, delivery_address, company_name, land_mark, mobile, order_no, date};
    ...
    const handleDelete = (item) => {
      ...
      const handleUpdate = (event) => {
        ...
        axios.put(`http://localhost:8000/api/delivery/${itemId}`, { full_name, delivery_address, company_name, land_mark, mobile, order_no, date }).then(() => {
          setData(data.map((item) => (item.id === itemId ? { id: itemId, full_name, delivery_address, company_name, land_mark, mobile, order_no, date } : item)));
          setIsEditing(false);
          setFull_name('');
          setDelivery_address('');
          setCompany_name('');
      ...
      const handleCreate = (event) => {
        event.preventDefault();
        axios.post('http://localhost:8000/api/service/', { full_name, delivery_address, company_name, land_mark, mobile, order_no, date }).then((response) => {
          setData([...data, { id: response.data.id, full_name, delivery_address, company_name, land_mark, mobile, order_no, date }]);
          setFull_name('');
          setDelivery_address('');
          setCompany_name('');
          setLand_mark('');
    ...
    const generatePDF = () => {
        ...
        const head = [['ID', 'Full Name', 'Delivery Address',
            'Company Name', 'Land Mark', 'Mobile', 'Order No', 'Date']];
        const elements = data.map(client => [client._id, client.full_name, client.delivery_address,
            client.company_name, client.land_mark, client.mobile, client.order_no, client.date]);
